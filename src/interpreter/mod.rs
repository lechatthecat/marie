...
use crate::value::constant::{VARTYPE_CONSTANT, VARTYPE_REASSIGNED};
use std::collections::HashMap;

pub fn interp_expr<'a>(scope: usize, env : &mut HashMap<(usize, i32, OranString<'a>), OranValue<'a>>, reduced_expr: &'a AstNode) -> OranValue<'a> {
    match reduced_expr {
        ...
        AstNode::Ident(ref ident) => {
            let val = &*env.get(&(scope, 0, OranString::from(ident))).unwrap_or_else(|| panic!("The variable \"{}\" is not defined.", ident));
            val.clone()
        }
        AstNode::Assign(ref var_type, ref ident, ref expr) => {
            let val = env.get(&(scope, 0, OranString::from(ident)));
            if *var_type == VARTYPE_CONSTANT && val != None {
                panic!("You can't assign value twice to a constant variable.");
            } else if *var_type == VARTYPE_REASSIGNED && OranVariable::from(val.unwrap()).var_type == VARTYPE_CONSTANT {
                panic!("You can't assign value twice to a constant variable.");
            }
            ...
            env.insert((scope, 0, OranString::from(ident)), oran_val.clone());
            oran_val
        }
        AstNode::FunctionCall(ref func_ast, ref name, ref arg_values) => {
            match func_ast {
                ...
                Function::NotDefault => {
                    let func = *&env.get(&(scope, 1, OranString::from(name))).unwrap();
                    ...
                    for i in 0..func.args.len() {
                        ...
                        env.insert((scope+1, 0, OranString::from(arg_name)), val);
                    }
        ...
        AstNode::FunctionDefine(ref func_name, ref args, ref astnodes, ref fn_return) => {
            ...
            env.insert((scope, 1,  OranString::from(func_name)), val.clone());
            val
        }
        AstNode::Argument(ref argument_name, ref val) => {
            let val = interp_expr(scope, env, val);
            env.insert((scope, 0, OranString::from(argument_name)), val);
